...
const API={
  ...
  assetUrl:'/api/asset',
  async init(){
    // Avoid noisy failed network requests when running locally without API
    if(location.hostname === '127.0.0.1' || location.hostname === 'localhost'){ this.enabled = false; return; }
...
async function loadExternalSiteData(){
  const candidates = ['/site-data.json','/images/site-data.json','/data/site-data.json'];
  for(const p of candidates){
    try{
      const res = await fetch(p,{cache:'no-cache'});
      if(!res.ok) continue;
...
async function initState(){
  ...
  Object.keys(DEF).forEach(k=>{
    ...
    if(extValue !== undefined && extValue !== null) S[k] = extValue;
...
function viewIUTSubsection(subsectionId){
  ...
  disp.innerHTML = `
    <div class="card" style="padding:1rem">
      ...
      <p style="color:var(--ink2)">${sub.text||''}</p>
